...
import { useDebouncedFn } from 'use-debounced'

/**
 * @param {RefObject<HTMLElement | null>} scrollHeightRef 스크롤 높이를 가지는 DOM, 생략시 body에서 상속되었다고 가정
 * @param {RefObject<HTMLElement | null>} anchorRef 실질적으로 스크롤되는 DOM, 생략시 body로부터 측정
...
export function useScrollPosition(
  ...
  // 디바운싱시켜 스크롤 이벤트 등록
  const handleScroll = useDebouncedFn(() => updatePosition(), scrollDebouncingDelay, [
    anchorRef,
    scrollHeightRef,
    BODY
  ])
